...
import { initialFleet, orbitalVelocity, periodSeconds, formatTime, orbitPoint, makeSample, validateMission, type Craft } from './model';

type Preferences = { favorites: string[]; orbits: boolean; grid: boolean };
...
export function mount(root: HTMLElement) {
  ...
  let preferences: Preferences = { favorites: [], orbits: true, grid: false };
  try {
    const saved = JSON.parse(localStorage.getItem('orbital.preferences') || 'null');
    if (saved && typeof saved === 'object') preferences = { favorites: Array.isArray(saved.favorites) ? saved.favorites.filter((id: unknown) => typeof id === 'string').slice(0, 24) : [], orbits: saved.orbits !== false, grid: saved.grid === true };
  ...
  const later = (fn: () => void, ms: number) => { const timer = setTimeout(() => { timeouts.delete(timer); if (!disposed) fn(); }, ms); timeouts.add(timer); };
  const savePreferences = () => { try { localStorage.setItem('orbital.preferences', JSON.stringify(preferences)); } catch { /* Device storage is not required. */ } };
  ...
  function event(title: string, description: string, symbol = 'check') {
  ...
  function renderFleet() {
    ...
    if (!visible.length) $('<p>', { class: 'empty-state' }).text(query ? 'No spacecraft match your search.' : 'No starred spacecraft yet. Star one in its details panel.').appendTo(list);
  ...
  function setSimulation(nextPaused: boolean, nextSpeed: number) {
    if (typeof nextPaused !== 'boolean' || ![1, 10, 60, 300].includes(nextSpeed)) throw new Error('Use a supported speed: 1, 10, 60, or 300.');
    paused = nextPaused; speed = nextSpeed;
  ...
  function showDialog(id: string) {
    const dialog = $root.find(`#${id}`)[0] as HTMLDialogElement;
    if (dialog.open) return;
    $root.find('dialog[open]').each(function () { (this as HTMLDialogElement).close(); });
    dialog.showModal();
  }
  ...
  function createMission(name: string, purpose: string, altitude: number, inclination: number) {
    const cleanName = name.trim().toUpperCase();
    const error = validateMission(cleanName, purpose, altitude, inclination, fleet);
    if (error) throw new Error(error);
    const index = fleet.length + 1;
    const craft: Craft = { id: `ORB-${String(index).padStart(3, '0')}`, name: cleanName, purpose, altitude, inclination, raan: (index * 47) % 360, phase: .4 + index * .55, color: '#c1f88b', samples: [] };
    craft.samples.push(makeSample(craft, seconds));
    fleet.push(craft);
    filter = 'all'; $root.find('#fleet-search').val('');
    globe?.sync(); select(craft.id);
  ...
  function renderTelemetryTable() {
    ...
    for (const sample of [...craft.samples].slice(-30).reverse()) $('<tr>').append(...[formatTime(sample.time), `${sample.signal.toFixed(1)} dBm`, `${sample.battery.toFixed(1)}%`, `${sample.altitude.toFixed(1)} km`].map(value => $('<td>').text(value))).appendTo(body);
  }
  function updateTelemetry() {
    ...
    drawChart();
    if (($root.find('#telemetry-dialog')[0] as HTMLDialogElement).open) renderTelemetryTable();
  }
  const chart = $root.find('#telemetry-chart')[0] as HTMLCanvasElement;
  function drawChart() {
    const ctx = chart.getContext('2d'); if (!ctx) return;
    const width = chart.clientWidth, height = chart.clientHeight; if (!width || !height) return;
    const ratio = Math.min(window.devicePixelRatio || 1, 2);
    if (chart.width !== Math.floor(width * ratio) || chart.height !== Math.floor(height * ratio)) { chart.width = Math.floor(width * ratio); chart.height = Math.floor(height * ratio); }
    ctx.setTransform(ratio, 0, 0, ratio, 0, 0); ctx.clearRect(0, 0, width, height);
    ctx.strokeStyle = '#263240'; ctx.lineWidth = 1; ctx.setLineDash([3, 5]);
    for (const y of [4, height / 2, height - 4]) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(width, y); ctx.stroke(); }
    ctx.setLineDash([]);
    ...
    const gradient = ctx.createLinearGradient(0, 0, 0, height); gradient.addColorStop(0, '#a5d77530'); gradient.addColorStop(1, '#a5d77500');
    ctx.beginPath(); ctx.moveTo(points[0].x, height); for (const p of points) ctx.lineTo(p.x, p.y); ctx.lineTo(points.at(-1)!.x, height); ctx.closePath(); ctx.fillStyle = gradient; ctx.fill();
    ctx.beginPath(); points.forEach((p, index) => index ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)); ctx.strokeStyle = '#b9ed89'; ctx.lineWidth = 1.7; ctx.lineJoin = 'round'; ctx.stroke();
    const latest = points.at(-1)!; ctx.beginPath(); ctx.arc(latest.x, latest.y, 3, 0, Math.PI * 2); ctx.fillStyle = '#d6ffaa'; ctx.shadowColor = '#c1f88b'; ctx.shadowBlur = 10; ctx.fill(); ctx.shadowBlur = 0;
  ...
  const chartObserver = new ResizeObserver(drawChart); chartObserver.observe(chart);
  function exportTelemetry() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `${craft.name.toLowerCase().replace(/[^a-z0-9_-]/g, '-')}-telemetry.csv`; a.click(); later(() => URL.revokeObjectURL(url), 1000);
    toast(`Telemetry exported for ${craft.name}.`);
  }
  const commands: { label: string; icon: string; hint: string; run: () => void }[] = [
  ...
  function renderCommands() {
    const matches = commandOptions(); cmdIndex = Math.min(cmdIndex, Math.max(0, matches.length - 1));
    ...
    if (!matches.length) $('<div>', { class: 'empty-state' }).text('No matching actions.').appendTo(list);
  }
  function showCommands() { $root.find('#command-search').val(''); cmdIndex = 0; renderCommands(); showDialog('command-dialog'); }
  function runCommand(index: number) { const command = commandOptions()[index]; if (!command) return; ($root.find('#command-dialog')[0] as HTMLDialogElement).close(); command.run(); }
  function action(name: string) {
    switch (name) {
      case 'planner': showPlanner(); break;
      case 'telemetry': renderTelemetryTable(); showDialog('telemetry-dialog'); break;
      case 'help': showDialog('help-dialog'); break;
      case 'command': showCommands(); break;
      case 'export': exportTelemetry(); break;
      case 'reset-view': globe?.reset(); toast('Camera centered on Earth.'); break;
      case 'orbits': preferences.orbits = !preferences.orbits; globe?.setOrbits(preferences.orbits); $root.find('[data-action=orbits]').toggleClass('active', preferences.orbits).attr('aria-pressed', String(preferences.orbits)); savePreferences(); break;
      case 'grid': preferences.grid = !preferences.grid; globe?.setGrid(preferences.grid); $root.find('[data-action=grid]').toggleClass('active', preferences.grid).attr('aria-pressed', String(preferences.grid)); savePreferences(); break;
      case 'expand': expanded = !expanded; $root.toggleClass('expanded', expanded); $root.find('[data-action=expand]').attr('aria-label', expanded ? 'Restore orbital view' : 'Expand orbital view').attr('aria-pressed', String(expanded)); break;
    }
  }
  $root.on('click.orbital', '[data-action]', function () { action(String($(this).data('action'))); });
  $root.on('click.orbital', '[data-view=overview]', () => { $root.find('dialog[open]').each(function () { (this as HTMLDialogElement).close(); }); if (expanded) action('expand'); document.getElementById('main')?.scrollIntoView({ behavior: reducedMotion ? 'instant' : 'smooth' }); });
  ...
  $root.on('click.orbital', '[data-close]', function () { ($(this).closest('dialog')[0] as HTMLDialogElement).close(); });
  $root.on('click.orbital', 'dialog', function (e) { if (e.target !== this) return; const rect = this.getBoundingClientRect(); if (e.clientX < rect.left || e.clientX > rect.right || e.clientY < rect.top || e.clientY > rect.bottom) (this as HTMLDialogElement).close(); });
  $root.on('input.orbital', '#mission-alt,#mission-inclination', previewMission);
  $root.on('submit.orbital', '#mission-form', function (e) {
    e.preventDefault();
    const form = new FormData(this as HTMLFormElement);
    try { createMission(String(form.get('name') || ''), String(form.get('purpose') || ''), Number(form.get('altitude')), Number(form.get('inclination'))); ($root.find('#mission-dialog')[0] as HTMLDialogElement).close(); (this as HTMLFormElement).reset(); previewMission(); }
    catch (error) { $root.find('#form-error').text(error instanceof Error ? error.message : 'The mission could not be created.'); }
  });
  ...
  $root.on('keydown.orbital', '#command-search', e => { const count = commandOptions().length; if (e.key === 'ArrowDown' || e.key === 'ArrowUp') { e.preventDefault(); cmdIndex = count ? (cmdIndex + (e.key === 'ArrowDown' ? 1 : -1) + count) % count : 0; renderCommands(); $root.find('.command-result.active')[0]?.scrollIntoView({ block: 'nearest' }); } if (e.key === 'Enter') { e.preventDefault(); runCommand(cmdIndex); } });
  $(document).on('keydown.orbital', e => {
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); showCommands(); return; }
    if (e.key === 'Escape' && expanded && !$root.find('dialog[open]').length) { action('expand'); return; }
    if ($(e.target).is('input,textarea,select,button,[contenteditable=true]') || $root.find('dialog[open]').length || e.metaKey || e.ctrlKey || e.altKey) return;
    if (e.code === 'Space') { e.preventDefault(); setSimulation(!paused, speed); }
    if (e.key.toLowerCase() === 'n') showPlanner();
  });
  ...
  try {
    ...
    globe.setGrid(preferences.grid); globe.setOrbits(preferences.orbits);
  } catch {
  ...
  select(selected); previewMission(); setSimulation(paused, speed);
  $root.find('#boot-status').text('Constellation connected');
  function frame(now: number) {
    const delta = document.hidden ? 0 : Math.min((now - last) / 1000, .15); last = now;
    if (!paused) { seconds += delta * speed; sampleClock += delta; }
    if (sampleClock >= 1) { sampleClock %= 1; for (const craft of fleet) { craft.samples.push(makeSample(craft, seconds)); if (craft.samples.length > 180) craft.samples.shift(); } updateTelemetry(); }
    uiClock += delta;
    if (uiClock > .1) { uiClock = 0; $root.find('#elapsed').text(formatTime(seconds)); $root.find('#utc-clock').text(`${new Date().toISOString().slice(11, 19)} UTC`); }
    if (!document.hidden) globe?.render(seconds);
  ...
  const objectInput = (value: unknown): Record<string, unknown> => { if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Expected an object.'); return value as Record<string, unknown>; };
  ...
  const tools: Tool[] = [
    { name: 'read_orbital_state', title: 'Read orbital simulation', description: 'Read the fictional constellation, selected spacecraft, simulation settings, and latest simulated telemetry.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: true }, execute: input => { if (Object.keys(objectInput(input)).length) throw new Error('This tool takes no arguments.'); return readState(); } },
    { name: 'select_spacecraft', title: 'Track a spacecraft', description: 'Select an existing spacecraft and update its highlighted orbit and visible telemetry.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: true }, execute: input => { const v = objectInput(input); if (typeof v.id !== 'string' || Object.keys(v).some(k => k !== 'id')) throw new Error('Provide a spacecraft id.'); select(v.id); return { selected }; } },
    { name: 'configure_orbital_simulation', title: 'Set simulation playback', description: 'Pause or resume the simulation and set its speed multiplier.', inputSchema: { type: 'object', properties: { paused: { type: 'boolean' }, speed: { type: 'number', enum: [1,10,60,300] } }, required: ['paused','speed'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: input => { const v = objectInput(input); if (typeof v.paused !== 'boolean' || typeof v.speed !== 'number' || Object.keys(v).some(k => !['paused','speed'].includes(k))) throw new Error('Provide a boolean paused value and a supported numeric speed.'); setSimulation(v.paused, v.speed); return { paused, speed }; } },
    { name: 'create_simulated_spacecraft', title: 'Create a simulated spacecraft', description: 'Create a fictional spacecraft in this page session, add its orbital path, and select it. No real launch, remote write, or spending occurs.', inputSchema: { type: 'object', properties: { name: { type: 'string', minLength: 2, maxLength: 24 }, purpose: { type: 'string', enum: ['Earth observation','Communications','Scientific research','Navigation'] }, altitude: { type: 'number', minimum: 250, maximum: 2000 }, inclination: { type: 'number', minimum: 0, maximum: 180 } }, required: ['name','purpose','altitude','inclination'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: true }, execute: input => { const v = objectInput(input); if (typeof v.name !== 'string' || typeof v.purpose !== 'string' || typeof v.altitude !== 'number' || typeof v.inclination !== 'number' || Object.keys(v).some(k => !['name','purpose','altitude','inclination'].includes(k))) throw new Error('Provide a name, purpose, altitude, and inclination.'); return createMission(v.name, v.purpose, v.altitude, v.inclination); } },
  ];
  if (context?.registerTool) for (const tool of tools) { try { void Promise.resolve(context.registerTool(tool, { signal: lifecycle.signal })).catch(() => { /* Optional integration must never interrupt the interface. */ }); } catch { /* Unsupported experimental implementation. */ } }

  return () => { disposed = true; cancelAnimationFrame(raf); timeouts.forEach(clearTimeout); lifecycle.abort(); chartObserver.disconnect(); globe?.dispose(); root.removeEventListener('asseterror', assetError); $root.off('.orbital'); $(document).off('.orbital'); $root.find('dialog[open]').each(function () { (this as HTMLDialogElement).close(); }); $root.removeClass('expanded'); };
}
